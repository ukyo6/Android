...
[![image](https://raw.githubusercontent.com/JsonChao/Awesome-Third-Library-Source-Analysis/master/ScreenShots/Glide%E6%A1%86%E6%9E%B6%E5%9B%BE.jpg)](https://raw.githubusercontent.com/JsonChao/Awesome-Third-Library-Source-Analysis/master/ScreenShots/Glide框架图.jpg)
...
解析 apt注解 生成的GlideModule, manifest配置的GlideModule, 以及builder中的配置
...
private static void initializeGlide(@NonNull Context context, @NonNull GlideBuilder builder) {
    ...
    // 1、反射调用apt生成的 GeneratedAppGlideModuleImpl 的构造方法
    GeneratedAppGlideModule annotationGeneratedModule = getAnnotationGeneratedGlideModules();
    // 2、如果GlideModule为空或者可配置manifest里面的标志为true，则获取manifest里面配置的GlideModule模块（manifestModules）
    ...
    // 3、初始化各种配置信息: 线程池 缓存大小
    Glide glide = builder.build(applicationContext);
    // 4、调用GlideModule # registerComponents()
    for (com.bumptech.glide.module.GlideModule module : manifestModules) { //manifest配置的
      module.registerComponents(applicationContext, glide, glide.registry);
    }
    if (annotationGeneratedModule != null) { //注解的
...
@NonNull
  Glide build(@NonNull Context context) {
    // 创建请求图片线程池sourceExecutor            //new GlideExecutor( new ThreadPoolExecutor(...)) 装饰模式
    if (sourceExecutor == null) {
      sourceExecutor = GlideExecutor.newSourceExecutor();
    }

    // 创建硬盘缓存线程池diskCacheExecutor, 池大小 = 1
    if (diskCacheExecutor == null) {
      diskCacheExecutor = GlideExecutor.newDiskCacheExecutor();
    }

    // 创建动画线程池animationExecutor, 池大小 = int maximumPoolSize = bestThreadCount >= 4 ? 2 : 1
    if (animationExecutor == null) {
      animationExecutor = GlideExecutor.newAnimationExecutor();
    }
	//计算最大内存缓存  
    	//Android 2.3.3及以前版本，像素点数据是保存在native memory，而bitmap对象是保存在Dalvik heap.
    	//从Android 3.0开始，像素点数据与bitmap对象一起存储在Dalvik heap中. 
    	//从8.0开始,像素点数据保存在native heap中 , 对象还在Delvik heap中.
    ...
    if (bitmapPool == null) {
      ...
      if (size > 0) {
        ...
        // 缓存策略在SDK大于19时，为SizeConfigStrategy，小于为AttributeStrategy。
...
[![image](https://raw.githubusercontent.com/JsonChao/Awesome-Android-Notebook/master/screenshots/Glide_with%E5%8A%A0%E8%BD%BD%E6%B5%81%E7%A8%8B.png)](https://raw.githubusercontent.com/JsonChao/Awesome-Android-Notebook/master/screenshots/Glide_with加载流程.png)
...
[![image](https://raw.githubusercontent.com/JsonChao/Awesome-Android-Notebook/master/screenshots/Glide_load%E5%8A%A0%E8%BD%BD%E6%B5%81%E7%A8%8B.png)](https://raw.githubusercontent.com/JsonChao/Awesome-Android-Notebook/master/screenshots/Glide_load加载流程.png)
...
private Request obtainRequest(
      ...
      int overrideWidth,
      int overrideHeight) {
    // 最终实际返回的是一个SingleRequest对象（将指定的资源加载进对应的Target)
...
可以看到，经过一连串的嵌套调用，最终执行到了decoder.decode()这行代码，decode是一个ResourceDecoder接口（资源解码器），根据不同的DataType和ResourceType它会有不同的实现类，这里的实现类是ByteBufferBitmapDecoder，接下来让我们来看看这个解码器内部的解码流程。
...
[![image](https://raw.githubusercontent.com/JsonChao/Awesome-Android-Notebook/master/screenshots/Glide%E5%8A%A0%E8%BD%BD%E6%B5%81%E7%A8%8B.png)](https://raw.githubusercontent.com/JsonChao/Awesome-Android-Notebook/master/screenshots/Glide加载流程.png)
