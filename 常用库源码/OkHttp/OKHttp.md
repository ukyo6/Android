...
[![image](https://upload-images.jianshu.io/upload_images/1916953-fc6439af2bfefddc.jpg?imageMogr2/auto-orient/)](https://upload-images.jianshu.io/upload_images/1916953-fc6439af2bfefddc.jpg?imageMogr2/auto-orient/)
...
Response getResponseWithInterceptorChain() throws IOException {
    ...
    // 请求时，对必要的Header进行一些添加，接收响应时，移除必要的Header ->对应client#cookieJar()
...
private int maxRequests = 64; //默认最大请求数64
private int maxRequestsPerHost = 5; //同一个host最大同时请求数5
...
private boolean promoteAndExecute() {
    ...
    synchronized (this) {
      for (Iterator<AsyncCall> i = readyAsyncCalls.iterator(); i.hasNext(); ) {
        ...
        // 满足条件的call加入到executableCalls中给线程池执行
        executableCalls.add(asyncCall);
        //将call加入到runningAsyncCalls中
        runningAsyncCalls.add(asyncCall);
      }
      isRunning = runningCallsCount() > 0;
    }
    
	//遍历executableCalls, 交给线程池执行
...
@Override public Response intercept(Chain chain) throws IOException {
    ...
    //如果设置了缓存目录
    if (cache != null) {
      //并且response的 header 里有"cache-control"
      if (HttpHeaders.hasBody(response) && CacheStrategy.isCacheable(response,   networkRequest)) {
        // Offer this request to the cache.
        // 缓存未经缓存过的response 通过DiskLruCache
...
private RealConnection findConnection(int connectTimeout, int readTimeout, int writeTimeout,
    ...
    synchronized (connectionPool) {
      ...
      if (!foundPooledConnection) {
        ...
        // 在连接池中如果没有该连接，则创建一个新的连接，并将其分配，这样我们就可以在握手之前进行中断
        route = selectedRoute;
...
```java
private final Deque<RealConnection> connections = new ArrayDeque<>();  //双向迭代器的队列
