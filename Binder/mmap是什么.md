...
linux内核使用`vm_area_struct`结构来表示一个独立的虚拟内存区域，由于每个不同质的虚拟内存区域功能和内部机制都不同，因此一个进程使用多个vm_area_struct结构来分别表示不同类型的虚拟内存区域。各个`vm_area_struct`结构使用链表或者树形结构链接，方便进程快速访问，如下图所示：
...
总而言之，常规文件操作需要从磁盘 ->内核页缓存 ->用户主存的两次数据拷贝。而mmap操控文件，只需要从磁盘 -> 用户主存的一次数据拷贝过程。说白了，**mmap的关键点是实现了用户空间和内核空间的数据直接交互而省去了空间不同数据不通的繁琐过程。**因此mmap效率更高。
