...
### 1.1  installExistingPackageAsUser
...
```Java
//拷贝之前, 先绑定 DefaultContainerService 服务, 绑定过了handler发送 MCS_BOUND 消息
case INIT_COPY:{   
                    ...
                    if (!mBound) {          									//未绑定服务就去绑定
                        ...
                        if (idx == 0) {
                            mHandler.sendEmptyMessage(MCS_BOUND);     //绑定了服务,handler发送消息MCS_BOUND
                        }
                    }
}
...
INIT_COPY主要是确保`DefaultContainerService`已bound,`DefaultContainerService`是一个应用服务，具体负责实现APK等相关资源文件在内部或外部存储器上的存储工作。

而`MCS_BOUND`中则执行了`params.startCopy()`这句,也是最关键的开始copy文件。
...
```java
  final boolean startCopy() {
            ...
            try {
                ...
                if (++mRetries > MAX_RETRIES) {  //最大重试次数4次
 ...
 public void handleStartCopy() throws RemoteException {
           int ret = PackageManager.INSTALL_SUCCEEDED;
            ...
            ...
                     ...
                     */
                    ret = args.copyApk(mContainerService, true);  //复制apk到data/app下
...
- 一是`installPackageLI(args, res)`, 这个方法具体执行了解析package和后续操作

- 执行完毕后会走到`bm.restoreAtInstall(res.pkg.applicationInfo.packageName, token)`,会调用backupservice的`restoreAtInstall`方法,而`restoreAtInstall`方法最终又会调用`PMS`的`finishPackageInstall()`方法,完成安装。
